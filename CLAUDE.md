...
- `import os` added at module level (was entirely missing)
- `DRAWIO_MAX_FILE_SIZE_KB` env guard (default 2048 KB) added to `_save_drawio()`
- `DRAWIO_SHARE=1` env flag: shareable URL comment injected before save
- Structured audit logging: `_audit()` + `ENABLE_AUDIT_LOG=1` flag; all 5 tools instrumented
- Python target upgraded: 3.8+ EOL → 3.11+
- `_esc()` coverage: attr type_hint, method name, params, return_type fixed in drawio_converter_enriched
...
- `_apply_uml_styles` post-processing implementation (currently a documented no-op stub)

---

**Last Updated:** 2026-05-28 (Advisory Sprint 3 — Python 3.11+, _esc() fixes, audit complete)
